...
T_stack newStack(int size) {
	...
	s.sp=0;
	s.nbMaxElt=size;
	CHECK_IF(s.data = (T_elt *) malloc(s.nbMaxElt * sizeof(T_elt)), NULL, "malloc newStack");
...
void push(T_elt e, T_stack *p) {
	...
	if(p->sp == p->nbMaxElt){
		CHECK_IF(p->data = (T_elt *) realloc(p->data, (p->nbMaxElt+STACK_THRESHOLD)*sizeof(T_elt)), NULL, "malloc newstack");
